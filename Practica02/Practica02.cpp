 ...
 * Implementación del algoritmo Extreme Segments.
...
int main()
{
  ...
  for(int i = 0; i <= cont; i++)
	...
	{
	  bool isExtremeSegment = true;
	  int initialOrientation = -2; // No tiene orientación con valor -2.
	  for(int k = 0; k <= cont; k++)
	    {
	      ...
	      Point q = puntos[j];
	      Point r = puntos[k];
	      if(initialOrientation == -2) initialOrientation = OrientationTest(p, q, r);
	      else
		{
		  int newOrientation = OrientationTest(p, q, r);
		  if(newOrientation != initialOrientation)
		    {
		      isExtremeSegment = false;
		      break;
		    }
		}
	    }
	  if(isExtremeSegment)
	    {
	      convexHull.push_back(puntos[i]);
	      convexHull.push_back(puntos[j]);
	    }
	}
    }
